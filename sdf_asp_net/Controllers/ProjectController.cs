...
namespace sdf_asp_net.Controllers
{
    ...
    public class ProjectController : Controller
    {
        ...
        { var connected = new List<string>();
            ...
            DataTable dtblChatstand2 = new DataTable();
            ...
            using (SqlConnection sqlCon = new SqlConnection(connectionString))
            {
                ...
                query = "SELECT Username FROM Chatstand WHERE Stand=@Stand";
                sqlDa = new SqlDataAdapter(query, sqlCon);
                sqlDa.SelectCommand.Parameters.AddWithValue("@Stand", Stand);
                sqlDa.Fill(dtblChatstand);
                query = "SELECT Username FROM Chatstand WHERE Stand=@Stand";
                sqlDa = new SqlDataAdapter(query, sqlCon);
                sqlDa.SelectCommand.Parameters.AddWithValue("@Stand", Stand2);
                sqlDa.Fill(dtblChatstand2);
            ...
            ProjectViewModel pvm = new ProjectViewModel();
                ...
                {
                    connected.Add(dtblChatstand.Rows[i][0].ToString());
                }
                pvm.Chatlist = connected;


            for (int i = 0; i < dtblChatstand2.Rows.Count; i++)

            {
                offline.Add(dtblChatstand2.Rows[i][0].ToString());
            }
            pvm.Chatlistoff = offline;

            ChatModel chatpvm = new ChatModel();
            for (int i = 0; i < dtblProjectUser.Rows.Count; i++)
            {
                Onlineusers.Add(dtblProjectUser.Rows[i][1].ToString());

            }
            chatpvm.OnlineMember = Onlineusers;
